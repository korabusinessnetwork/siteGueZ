...
const container = 'mx-auto max-w-6xl px-4 md:px-8'

export default function Home() {
  return (
    <>
      {/* HERO — mobile (imagem com overlay) */}
      <section className="md:hidden relative h-[460px]">
        <ImagePlaceholder
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-marrom/90 via-marrom/40 to-marrom/10" />
        <div className="absolute inset-x-0 bottom-0 p-5">
          <h1 className="font-display text-[28px] leading-tight text-creme mb-2.5">
            Toda marca tem uma história que merece ser contada de verdade
          </h1>
          <p className="text-sm leading-snug text-creme-sobre-escuro mb-4">
      ...
      {/* HERO — desktop (split) */}
      <section className="hidden md:grid grid-cols-2 items-stretch">
      ...
      {/* QUEM É */}
      <section className={`${container} py-10 md:py-16 md:grid md:grid-cols-2 md:gap-14 md:items-center`}>
        <div>
          <Eyebrow>Quem é a GueZ</Eyebrow>
          <p className="font-display text-[19px] md:text-[30px] leading-snug mt-2 mb-4 md:mb-5">
            Somos a Lucy e a Érica: storytelling, marketing e fotografia, de perto e com alma.
          </p>
          <p className="hidden md:block text-[15px] leading-relaxed text-marrom-2 max-w-[480px]">
          ...
          {[
            ...
            <div key={p.nome} className="text-center flex-1">
              <ImagePlaceholder
                label={p.label}
                circle
                className="w-[76px] h-[76px] md:w-[140px] md:h-[140px] mx-auto"
              />
              <div className="text-[13px] md:text-[15px] font-semibold mt-2 md:mt-3">{p.nome}</div>
              <div className="text-[11.5px] md:text-[13px] text-marrom-label">{p.papel}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="grid gap-3 md:grid-cols-3 md:gap-6">
          {SERVICOS_RESUMO.map((c) => (
            <div
              ...
              className={
                c.destaque
                  ? 'bg-marrom text-creme rounded-md md:rounded-lg p-[18px] md:p-8'
                  : 'bg-papel border border-linha rounded-md md:rounded-lg p-[18px] md:p-8'
              }
            >
              <div className="font-display text-base md:text-[22px] mb-1 md:mb-2.5">{c.titulo}</div>
      ...
      <section className={`${container} pb-10 md:pb-16`}>
        <div className="bg-creme-escuro rounded-lg p-[22px] md:p-12 md:grid md:grid-cols-[1fr_repeat(3,1fr)] md:gap-8 md:items-center">
          <div className="font-display text-lg md:text-[26px] leading-snug mb-3.5 md:mb-0">
            Como funciona o real time
          </div>
          {REAL_TIME_PASSOS.map((p, i) => (
            <div key={i} className="flex gap-3 items-start mt-3 md:mt-0">
      ...
      <section className={`${container} pb-10 md:pb-16`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 md:gap-5">
          {PORTFOLIO_DESTAQUE.map((item) => (
            <div key={item.id} className="relative rounded-md md:rounded-lg overflow-hidden bg-papel border border-linha">
              <div className="relative">
                <ImagePlaceholder
                  label={item.placeholder}
                  bordered={false}
                  rounded="rounded-none"
                  className="h-[100px] md:h-[180px] w-full"
                />
                {item.isRealtime && (
                  <span className="absolute top-1.5 left-1.5 md:top-2.5 md:left-2.5 bg-papel rounded-pill px-1.5 md:px-2.5 py-0.5 md:py-1 text-[8.5px] md:text-[10px] font-bold text-laranja-cta">
                    REAL TIME
                  </span>
                )}
              </div>
              <div className="px-2.5 md:px-3.5 py-2 md:py-3 text-[11.5px] md:text-[13.5px] font-semibold">
                {item.titulo}
              </div>
            </div>
          ))}
      ...
      <section className={`${container} pb-10 md:pb-16`}>
        ...
        <div className="grid gap-3 md:grid-cols-2 md:gap-6">
          {DEPOIMENTOS.map((d, i) => (
            <div key={i} className="bg-papel border border-linha rounded-md md:rounded-lg p-[18px] md:p-7">
              <p className="font-display italic text-[14.5px] md:text-[18px] leading-normal mb-2.5 md:mb-3.5">
                “{d.texto}”
              </p>
      ...
      <section className={`${container} pb-12 md:pb-20`}>
        <div className="bg-marrom text-creme rounded-lg p-6 md:p-14 md:grid md:grid-cols-2 md:gap-12 md:items-center">
          <div>
            <div className="font-display text-[19px] md:text-[30px] mb-2 md:mb-3">Bora contar essa história?</div>
          ...
          <div className="flex flex-col md:flex-row gap-2.5 md:gap-3">
            <input
              ...
              aria-label="Seu nome"
              className="flex-1 rounded-sm px-3.5 py-3 bg-creme text-marrom placeholder:text-marrom-label border-0 text-sm"
            />
            <input
